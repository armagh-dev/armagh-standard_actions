...
require_relative '../../../lib/armagh/standard_actions/text_publish_action'

class TestTextPublishAction < Test::Unit::TestCase
  ...
  def setup
    ...
    @config = Armagh::StandardActions::TextPublishAction.create_configuration( [], 'text_test', @config_values )
    @text_publish_action = Armagh::StandardActions::TextPublishAction.new(@caller, @logger, @config, collection)
    @id = '123'
    @content = {'bson_binary' => ''}
    @metadata = {},
    @time = Time.now
