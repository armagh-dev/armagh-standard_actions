...
require_relative '../../lib/armagh/standard_actions/newsml_publish_action'

class TestIntegrationNewsmlPublishAction < Test::Unit::TestCase
  ...
  def setup
    ...
    @config = Armagh::StandardActions::NewsmlPublishAction.create_configuration([], 'test', @config_values)
    @newsml_publish_action = Armagh::StandardActions::NewsmlPublishAction.new( @caller, @logger, @config, @collection )
    @id = '123'
    @content = {'bson_binary' => ''}
    @metadata = {'meta' => true}
    @source = 'news source'
