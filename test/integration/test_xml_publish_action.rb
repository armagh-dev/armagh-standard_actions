...
require_relative '../../lib/armagh/standard_actions/xml_publish_action'

class TestIntegrationXmlPublishAction < Test::Unit::TestCase
  ...
  def setup
    ...
    @config = Armagh::StandardActions::XmlPublishAction.create_configuration([], 'xml_test', @config_values)
    @xml_publish_action = instantiate_action(Armagh::StandardActions::XmlPublishAction, @config)
  ...
  def test_publish_sets_document_contents_and_attributes
    ...
    assert_equal "Copyright (C) 2016 PR Newswire. All rights reserved\n", @doc.copyright
  end
